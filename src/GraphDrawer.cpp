...
void GraphDrawer::DrawGraph(std::vector<point> R2points, std::vector<vertex> vertexes)
{
    int canvas_size_x = 0;
    int canvas_size_y = 0;
    ...
    for (int f = 0; f < vertexes.size(); f++)
    {
        ...
        for (int number = 0; number < symbols.size(); number ++)
        {
            ...
            for (int y = 0; y < 16; y++)
            {
                for (int x = 0; x < 16; x++)
                {
                    if ((int)font_data[symbol][y][x].r != 255 && (int)font_data[symbol][y][x].g != 255 && (int)font_data[symbol][y][x].b != 255)
                    {
        ...
        for (int s = 0; s < vertexes[f].neighbours.size(); s++)
        {
            ...
            for (int y = min_vertex_y; y < max_vertex_y; y ++ )
            {
                ...
                {
                    if (abs((y_2 - y_1) * x + (x_1 - x_2) * y + y_1*x_2 - x_1*y_2)/(pow(pow((y_2 - y_1), 2) + pow((x_1 - x_2), 2), 0.5)) < 1)
                    {
                        if (pow(pow(x - x_2, 2) + pow(y - y_2, 2), 0.5) > 6 && pow(pow(x - x_1, 2) + pow(y - y_1, 2), 0.5) > 6)
                        {
                            to_paint[y].push_back(x);
                        }
                    }
