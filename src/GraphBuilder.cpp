...
void GraphBuilder::CountConstraints(std::map<int, std::map<int, std::vector<int>>> distances)
{
...
double GraphBuilder::GetEnergyDiff_X(int m)
{
    ...
    for (int i = 0; i < R2points.size(); i++)
    {
        ...
        pow(pow((R2points[m].x - R2points[i].x), 2) + pow((R2points[m].y - R2points[i].y), 2) , 0.5));
        }
    }
    return Energy;
}
...
double GraphBuilder::GetEnergySecondDiff_XX(int m)
{
    ...
    for (int i = 0; i < R2points.size(); i++)
    {
        ...
        {
        Energy += k[m][i] * (1 -
        (l[m][i]*pow((R2points[m].y - R2points[i].y), 2))/
        pow((pow((R2points[m].x - R2points[i].x), 2) + pow((R2points[m].y - R2points[i].y), 2)), 1.5));
        }
    }
    return Energy;
}
...
double GraphBuilder::GetEnergySecondDiff_XY(int m)
{
    ...
    for (int i = 0; i < R2points.size(); i++)
    {
        ...
        {
        Energy += k[m][i] * (
        (l[m][i]*(R2points[m].y - R2points[i].y) * (R2points[m].x - R2points[i].x))/
        pow(pow((R2points[m].x - R2points[i].x), 2) + pow((R2points[m].y - R2points[i].y), 2) , 1.5));
        }
    }
    return Energy;
...
double GraphBuilder::GetEnergySecondDiff_YY(int m)
{
    ...
    for (int i = 0; i < R2points.size(); i++)
    {
        ...
        {
        Energy += k[m][i] * (1 -
        (l[m][i]*pow((R2points[m].x - R2points[i].x), 2))/
        pow(pow((R2points[m].x - R2points[i].x), 2) + pow((R2points[m].y - R2points[i].y), 2) , 1.5));
        }
    }
    return Energy;
...
{   
    ...
    while (true)
    {
        ...
        {
            double new_delta = pow(pow(GetEnergyDiff_X(m), 2) + pow(GetEnergyDiff_Y(m), 2), 0.5);
